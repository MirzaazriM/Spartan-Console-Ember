define('ember-paper/components/paper-menu-content', ['exports', 'ember-paper/templates/components/paper-menu-content', 'ember-basic-dropdown/components/basic-dropdown/content', 'ember-css-transitions/mixins/transition-mixin'], function (exports, _paperMenuContent, _content, _transitionMixin) {
  ...
  exports.default = _content.default.extend({
    ...
    style: Ember.computed('top', 'left', 'right', 'transform', 'transformOrigin', function () {
      let style = '';
    ...
    startObservingDomMutations() {
      if (MutObserver) {
        this.mutationObserver = new MutObserver(mutations => {
          ...
          let addedNodes = Array.prototype.slice.call(mutations[0].addedNodes).filter(node => !Ember.$(node).hasClass('md-ripple') && node.nodeName !== '#comment' && !(node.nodeName === '#text' && node.nodeValue === ''));
          let removedNodes = Array.prototype.slice.call(mutations[0].removedNodes).filter(node => !Ember.$(node).hasClass('md-ripple') && node.nodeName !== '#comment');
