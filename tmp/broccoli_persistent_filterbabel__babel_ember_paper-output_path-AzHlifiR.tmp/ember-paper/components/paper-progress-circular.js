define('ember-paper/components/paper-progress-circular', ['exports', 'ember-paper/templates/components/paper-progress-circular', 'ember-paper/mixins/color-mixin', 'ember-paper/utils/clamp'], function (exports, _paperProgressCircular, _colorMixin, _clamp) {
  'use strict';
  ...
  exports.default = Ember.Component.extend(_colorMixin.default, {
    ...
    isIndeterminate: Ember.computed.equal('mode', MODE_INDETERMINATE).readOnly(),

    strokeWidth: Ember.computed('strokeRatio', 'diameter', function () {
      return this.get('strokeRatio') * this.get('diameter');
    }),

    strokeDasharray: Ember.computed('mode', 'diameter', 'strokeWidth', function () {
    ...
    d: Ember.computed('diameter', 'strokeWidth', 'isIndeterminate', function () {
      return this.getSvgArc(this.get('diameter'), this.get('strokeWidth'), this.get('isIndeterminate'));
    }),

    pathDiameter: Ember.computed('diameter', 'strokeWidth', function () {
      return this.get('diameter') - this.get('strokeWidth');
    }),

    containerStyle: Ember.computed('diameter', function () {
    ...
    svgStyle: Ember.computed('diameter', function () {
      let diameter = this.get('diameter');
      let width = `width: ${diameter}px`;
      let height = `height: ${diameter}px`;
      let transformOrigin = `transform-origin: ${diameter / 2}px ${diameter / 2}px ${diameter / 2}px`;
      return Ember.String.htmlSafe([width, height, transformOrigin].join('; '));
    }),

    pathStyle: Ember.computed('strokeWidth', function () {
      return Ember.String.htmlSafe(`stroke-width: ${this.get('strokeWidth')}px`);
    }),
    ...
    didReceiveAttrs() {
      this._super(...arguments);
      let newValue = (0, _clamp.default)(this.get('value'), 0, 100);
    ...
    renderCircle(animateFrom, animateTo, ease = linearEase, animationDuration = 100, iterationCount = 0, dashLimit = 100) {
      ...
      if (animateTo === animateFrom) {
        ...
        let animation = () => {
          let currentTime = (0, _clamp.default)(now() - startTime, 0, animationDuration);
