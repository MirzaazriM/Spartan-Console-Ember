define('ember-paper/components/paper-tabs', ['exports', 'ember-paper/templates/components/paper-tabs', 'ember-composability-tools', 'ember-paper/mixins/color-mixin', 'ember-invoke-action'], function (exports, _paperTabs, _emberComposabilityTools, _colorMixin, _emberInvokeAction) {
  'use strict';

  Object.defineProperty(exports, "__esModule", {
    value: true
  });
  exports.default = Ember.Component.extend(_emberComposabilityTools.ParentMixin, _colorMixin.default, {
    ...
    _selectedTab: Ember.computed('childComponents.@each.value', 'selected', function () {
      return this.get('childComponents').findBy('value', this.get('selected'));
    }),

    _selectedTabDidChange: Ember.observer('_selectedTab', function () {
    ...
    inkBarLeft: Ember.computed('_selectedTab.left', function () {
      return this.get('_selectedTab.left') || 0;
    }),

    inkBarRight: Ember.computed('wrapperWidth', '_selectedTab.currentWidth', 'inkBarLeft', function () {
      return this.get('wrapperWidth') - this.get('inkBarLeft') - (this.get('_selectedTab.currentWidth') || 0);
    }),

    tabsWidth: Ember.computed('childComponents.@each.width', function () {
      return this.get('childComponents').reduce((prev, t) => prev + t.get('width'), 0);
    }),

    shouldPaginate: false,

    shouldCenter: Ember.computed('shouldPaginate', 'center', function () {
      return !this.get('shouldPaginate') && this.get('center');
    }),

    shouldStretch: Ember.computed('shouldPaginate', 'currentStretch', function () {
    ...
    fixOffsetIfNeeded() {
      ...
      if (tabRight - currentOffset > canvasWidth) {
        ...
        this.set('paginationStyle', Ember.String.htmlSafe(`transform: translate3d(-${newOffset}px, 0px, 0px);`));
      }
    ...
    currentOffset: 0,
    canPageBack: Ember.computed.gt('currentOffset', 0),
    canPageForward: Ember.computed('wrapperWidth', 'currentOffset', 'canvasWidth', function () {
    ...
    actions: {
      previousPage() {
        ...
        if (tab) {
          ...
          this.set('paginationStyle', Ember.String.htmlSafe(`transform: translate3d(-${left}px, 0px, 0px);`));
        }
      },
      ...
      nextPage() {
        ...
        if (tab) {
          ...
          this.set('paginationStyle', Ember.String.htmlSafe(`transform: translate3d(-${tab.get('left')}px, 0px, 0px);`));
        }
      },
      ...
      onChange(selected) {
        ...
        if (this.get('onChange')) {
          (0, _emberInvokeAction.invokeAction)(this, 'onChange', selected.get('value'));
        } else {
